...
fichier_res = "MPM-"  + "E" + str(int(E)) + "-nu" + "{:.2f}".format(nu) + "-g" + str(int(g))+ "_deux_cotes.res"
    ...
    for i, j in grid_m:        
        if grid_m[i, j] > 0:  # No need for epsilon here
            ...
            if i < 3 and (j >= indice_bas and j <= indice_haut):
                grid_v[i, j][0] = 0
                grid_v[i, j][1] = 0
            if i > n_grid - 3 and (j >= indice_bas and j <= indice_haut):
                grid_v[i, j][0] = 0
                grid_v[i, j][1] = 0            
...
def initialize():
    for i in range(n_particles):
        ...
        if x[i][0] > 0.5 * length_beam and x[i][0] < 0.52 and indice_temoin[None] == 0: 
...
#ecriture fichier
#fich = open("fichier_plot_beam_MPM_deux_cotes.txt", "w" )
scatter_plot_MPM = []
liste_p = []
#x_numpy = x.to_numpy()
